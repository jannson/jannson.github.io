...
// Parts (C) Gregor Richards, 2008
//
// 2008-11      - (Gregor Richards) Added more control codes: A, B, C, D
// 2007-08-12	- refresh():
...
//			cursor.
//	clrtosol()	Clears the portion of the current line before the
//			cursor.
//	clrline()	Clears the entire current line.
...
VT100.prototype.clrtosol = function()
{
	var r = this.row_;
	if (r >= this.ht_)
		return;
	for (var c = this.col_; c >= 0; --c) {
		this.text_[r][c] = ' ';
		this.attr_[r][c] = this.bkgd_;
	}
}

VT100.prototype.clrline = function()
{
	var r = this.row_;
	if (r >= this.ht_)
		return;
	for (var c = 0; c < this.wd_; ++c) {
		this.text_[r][c] = ' ';
		this.attr_[r][c] = this.bkgd_;
	}
}
	...
	for (i = 0; i < stuff.length; ++i) {
		...
		switch (this.esc_state_) {
			...
			switch (ch) {
				...
				break;
                            case 'A': // move up
                                var by = (this.csi_parms_[0]==0) ? 1 : this.csi_parms_[0];
                                this.move(this.row_-by, this.col_);
                                break;
                            case 'B': // move down
                                var by = (this.csi_parms_[0]==0) ? 1 : this.csi_parms_[0];
                                this.move(this.row_+by, this.col_);
                            ...
                            case 'C': // move forward
                                var by = (this.csi_parms_[0]==0) ? 1 : this.csi_parms_[0];
                                this.move(this.row_, this.col_+by);
                                break;
                            case 'D': // move back
                                var by = (this.csi_parms_[0]==0) ? 1 : this.csi_parms_[0];
                                this.move(this.row_, this.col_-by);
